function [x,y,typ]=MB_READ_INPUT_STATUS(job,arg1,arg2)
...
select job
  ...
  case 'set' then
    ...
    while %t do
      ...
      getvalue('Set MB_READ_INPUT_STATUS block parameters',..
               ['Slave ID';'Start Address';'Input Count'],..
  ...
  case 'define' then
    ...
    // private parameter
    fcode=hex2dec('02') // modbus function code
    ...
    gr_i=list(["xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.6,[""Modbus""],sz(1)*0.8,sz(2)*0.3,''fill'');";..
               "xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.1,[""0x02""],sz(1)*0.8,sz(2)*0.3,''fill'');"],8)

    x=standard_define([2,2],model,exprs,gr_i)
    x.graphics.id="Read Input Status"
end
