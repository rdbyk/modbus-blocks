function [x,y,typ]=MB_READ_HOLDING_REGS(job,arg1,arg2)
...
select job
  ...
  case 'set' then
    ...
    while %t do
      ...
      getvalue('Set MB_READ_HOLDING_REGS block parameters',..
               ['Slave ID';'Start Address';'Register Count'],..
  ...
  case 'define' then
    ...
    // private parameter
    fcode=hex2dec('03') // modbus function code
    ...
    gr_i=list(["xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.6,[""Modbus""],sz(1)*0.8,sz(2)*0.3,''fill'');";..
               "xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.1,[""0x03""],sz(1)*0.8,sz(2)*0.3,''fill'');"],8)

    x=standard_define([2,2],model,exprs,gr_i)
    x.graphics.id="Read Holding Registers"
end
