function [x,y,typ]=MB_PRESET_MULTIPLE_REGS(job,arg1,arg2)
...
select job
  ...
  case 'set' then
    ...
    while %t do
      [ok,slave,addr,exprs]=..
      getvalue('Set MB_PRESET_MULTIPLE_REGS block parameters',..
  ...
  case 'define' then
    ...
    // private parameter
    fcode=hex2dec('10'); // modbus function code
    ...
    gr_i=list(["xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.6,[""Modbus""],sz(1)*0.8,sz(2)*0.3,''fill'');";..
               "xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.1,[""0x10""],sz(1)*0.8,sz(2)*0.3,''fill'');"],8)

    x=standard_define([2,2],model,exprs,gr_i)
    x.graphics.id="Preset Multiple Registers"
end
